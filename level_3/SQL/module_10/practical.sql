...
CREATE DATABASE ITOL_module10_db;
USE ITOL_module10_db;
...
-- Apparently TSQL does not support BEFORE triggers. https://stackoverflow.com/questions/642822/how-can-i-do-a-before-updated-trigger-with-sql-server
...
BEGIN
    ...
    OPEN product_cursor;
	FETCH NEXT
	FROM product_cursor INTO @product_id, @product_price
	WHILE @@FETCH_STATUS = 0 -- Apparently @@FETCH_STATUS is global, for all cursors open on the current connection. Avoid using, look for alternatives
		BEGIN
...
INSERT INTO [dbo].[orders] (customer_name, order_amount) VALUES ('Alice', 100), ('Bob', 200);
INSERT INTO [dbo].[customers] (customer_name, email) VALUES ('Alice', 'alice@example.com'), ('Bob', 'bob@example.com');

EXEC dynamic_select 'orders';
EXEC dynamic_select 'customers';
...
EXEC process_transaction '1234567890', 500;
EXEC process_transaction '1234567890', 500;
